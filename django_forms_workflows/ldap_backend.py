...
class ConfigurableLDAPBackend(LDAPBackend):
    ...
    def _sync_profile_attributes(self, user, profile_fields):
        ...
        try:
            profile_model_path = getattr(
                settings,
                'FORMS_WORKFLOWS_LDAP_PROFILE_MODEL',
                None
            )
            ...
            logger.error(f"Error syncing profile attributes for {user.username}: {str(e)}")
    ...
    def _should_sync_groups(self):
        ...
        return getattr(
            settings,
            'FORMS_WORKFLOWS_LDAP_SYNC_GROUPS',
            True
        )
...
def get_user_manager(user):
    ...
    try:
        ...
        username_attr = getattr(settings, 'FORMS_WORKFLOWS_LDAP_USERNAME_ATTR', 'sAMAccountName')
                ...
                if manager_result and len(manager_result) > 0:
                    ...
                    if username_attr in manager_attrs:
                        ...
                        try:
                            ...
                            logger.warning(f"Manager {manager_username} not found in Django database")
                            return None
    ...
    try:
        ...
        for dn, attrs in result:
            if dn:  # Skip referrals
                user_info = {
                    'username': attrs.get(username_attr, [b''])[0].decode('utf-8') if attrs.get(username_attr) else '',
                    'first_name': attrs.get('givenName', [b''])[0].decode('utf-8') if attrs.get('givenName') else '',
                    'last_name': attrs.get('sn', [b''])[0].decode('utf-8') if attrs.get('sn') else '',
    ...
    try:
        ...
        if result and len(result) > 0:
            ...
            for key, values in attrs.items():
                decoded_attrs[key] = [v.decode('utf-8') if isinstance(v, bytes) else v for v in values]
