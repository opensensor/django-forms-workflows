...
def workflow_builder_view(request, form_id):
    ...
    return render(request, 'admin/django_forms_workflows/workflow_builder.html', context)
...
def workflow_builder_save(request):
    ...
    try:
        ...
        if not form_id:
            return JsonResponse({'success': False, 'error': 'Form ID is required'}, status=400)
...
def convert_visual_to_workflow(workflow_data, form_definition):
    ...
    existing_actions = {action.name: action for action in form_definition.post_actions.all()}
    ...
    for node in action_nodes:
        ...
        if action_type == 'database':
            ...
            action_data['db_field_mappings'] = config.get('field_mappings', config if isinstance(config, list) else [])
        elif action_type == 'ldap':
            # Handle new structured config format
            action_data['ldap_dn_template'] = config.get('ldap_dn_template', '')
            action_data['ldap_field_mappings'] = config.get('field_mappings', config if isinstance(config, list) else [])
