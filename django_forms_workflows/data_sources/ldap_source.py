...
class LDAPDataSource(DataSource):
    ...
    def get_value(self, user, field_name: str, **kwargs) -> Optional[Any]:
        ...
        if not self.is_available():
            logger.warning("LDAP data source is not available (django-auth-ldap not configured)")
            return None
    ...
    def _get_manager_email(self, ldap_user) -> Optional[str]:
        """
        ...
        try:
            ...
            # For now, return None
            logger.debug(f"Manager DN found: {manager_dn}, but email lookup not implemented")
            return None
